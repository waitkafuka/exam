...
$.widget("ui.selectWidget", {
    options: {
        ...
        function v(t) {
            ...
            e.val(u.eq(t).val());
            n.text(u.eq(t).text()).attr('title',u.eq(t).text());
            a = t;
            if (p) {
                e.change(u.eq(t).val());
                p(e.val(),e)
        ...
        if (!o) {
            ...
            n.text(u.first().text()).attr('title',u.first().text());
        } else {
            e.val(e.find("option[selected]").val());
            $("li", s).eq(e.find("option[selected]").index()).addClass("active");
            n.text(e.find("option[selected]").text()).attr('title',e.find("option[selected]").text());
        }
